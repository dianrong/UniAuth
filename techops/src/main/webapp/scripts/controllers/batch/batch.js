define([ '../../utils/constant' ], function(constant) {
      var Controller = function($rootScope, $scope, $location, AlertService, FileUploader, GroupService, TagService,
          RoleService) {
        // params
        ...
        $scope.process_btn_enable = function() {
          ...
          if (fileItems && fileItems.length > 0) {
            var handler = batch_process_handler.getHandler();
            if (handler) {
        ...
        // 是否展示包含子组的checkBox.
        $scope.is_show_sub_grp_check = function() {
          if ($scope.process_param.type) {
            var handler = batch_process_handler.getHandler();
            if (handler && handler.show_sub_grp_check) {
              return handler.show_sub_grp_check();
            }
          }
          return false;
        }
        ...
        uploader.onBeforeUploadItem = function(fileItem) {
          ...
          if (handler && handler.addParams) {
            batch_process_handler.getHandler().addParams(fileItem);
          }
