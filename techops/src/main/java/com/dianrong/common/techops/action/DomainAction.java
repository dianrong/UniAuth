...
public class DomainAction {
    ...
    @PreAuthorize("hasRole('ROLE_SUPER_ADMIN') and principal.permMap['DOMAIN'] != null and principal.permMap['DOMAIN'].contains('techops')")
    public Response<StakeholderDto>  addNewStakeholder(@RequestBody StakeholderParam stakeholderParam) {
        return uARWFacade.getDomainRWResource().addNewStakeholder(stakeholderParam);
    }

    @RequestMapping(value = "/stakeholder-delete" , method= RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    @PreAuthorize("hasRole('ROLE_SUPER_ADMIN') and principal.permMap['DOMAIN'] != null and principal.permMap['DOMAIN'].contains('techops')")
    ...
    @RequestMapping(value = "/stakeholder-modify" , method= RequestMethod.POST, produces = MediaType.APPLICATION_JSON_VALUE)
    @PreAuthorize("hasRole('ROLE_SUPER_ADMIN') and principal.permMap['DOMAIN'] != null and principal.permMap['DOMAIN'].contains('techops')")
