...
@Slf4j
@Service
public class SynchronousDb extends TenancyBasedService {
   ...
   */
  @Transactional
  public void dbProcess(DepartmentList departmentList, JobList jobList,
    ...
    GroupProcessUtil.groupProcess(departmentList.content(), new GroupProcess<HrDept>() {
      @Override
      public void process(List<HrDept> groupList) {
    ...
    GroupProcessUtil.groupProcess(jobList.content(), new GroupProcess<HrJob>() {
      @Override
      public void process(List<HrJob> groupList) {
        if (ObjectUtil.collectionIsEmptyOrNull(groupList)) {
          return;
        }
    ...
    GroupProcessUtil.groupProcess(legalEntityList.content(), new GroupProcess<HrLe>() {
      @Override
      public void process(List<HrLe> groupList) {
        if (ObjectUtil.collectionIsEmptyOrNull(groupList)) {
          return;
        }
    ...
    GroupProcessUtil.groupProcess(personList.content(), new GroupProcess<HrPerson>() {
      @Override
      public void process(List<HrPerson> groupList) {
        if (ObjectUtil.collectionIsEmptyOrNull(groupList)) {
          return;
        }
