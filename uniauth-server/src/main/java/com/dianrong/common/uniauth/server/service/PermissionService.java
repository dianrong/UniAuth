...
public class PermissionService extends TenancyBasedService {
    ...
    public PermissionDto addNewPerm(PermissionParam permissionParam) {
        ...
        // 同一个域下面不能出现重复数据
        dataFilter.addFieldsCheck(FilterType.FILTER_TYPE_EXSIT_DATA, FilterData.buildFilterData(FieldType.FIELD_TYPE_VALUE, value),
                FilterData.buildFilterData(FieldType.FIELD_TYPE_PERM_TYPE_ID, permTypeId), FilterData.buildFilterData(FieldType.FIELD_TYPE_DOMAIN_ID, domainId));
    ...
    public void updatePerm(PermissionParam permissionParam) {
        ...
        if ((status != null && status == AppConstants.STATUS_ENABLED) || status == null) {
            // 同一个域下面不能出现重复数据
            dataFilter.updateFieldsCheck(permissionParam.getId(), FilterData.buildFilterData(FieldType.FIELD_TYPE_VALUE, value),
                    FilterData.buildFilterData(FieldType.FIELD_TYPE_PERM_TYPE_ID, permTypeId), FilterData.buildFilterData(FieldType.FIELD_TYPE_DOMAIN_ID, domainId));
        }
    ...
    public PageDto<PermissionDto> searchPerm(List<Integer> permIds, Integer permId, Integer domainId, Byte status, String valueExt, String value, Integer permTypeId,
