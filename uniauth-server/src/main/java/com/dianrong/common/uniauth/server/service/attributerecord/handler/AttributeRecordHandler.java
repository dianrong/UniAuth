...
public interface AttributeRecordHandler {
  ...
  /**
   * 在调用目标方法之前回调的方法.
   * @param identity 比如userId, groupId等,不能为空.
   * @param extendId 扩展属性id, 不能为空. 
   * @return 扩展属性操作前的值.
   * @throws InvalidParameterTypeException 如果传入的identity和extendId类型不合法.
   */
  ExtendVal invokeTargetBefore(Object identity, Object extendId);
   ...
   * @throws InvalidParameterTypeException 如果传入的identity和extendId类型不合法.
   */
  AttributeRecords invokeTargetAfter(Object identity, Object extendId, ExtendVal originalVal, Throwable throwable);
}
