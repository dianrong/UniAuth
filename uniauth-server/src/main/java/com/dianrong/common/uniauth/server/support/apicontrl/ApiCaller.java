...
public class ApiCaller implements CallerCredential<ApiCtlPermission> {
    ...
    public ApiCaller(String domainName, String  domainCode, Set<ApiCtlPermissionItem> permissions, Long expiredTime){
        Assert.notNull(domainCode);
        this.createTime = System.currentTimeMillis();
        if (expiredTime == null) {
            this.expiredTime = this.createTime + AppConstants.DEFAULT_API_CALL_TOKEN_AVAILABLE_MILLiSECONDS;
        } else {
            if (expiredTime < this.createTime) {
                throw new IllegalArgumentException(expiredTime + " is not a valid expired time, because it is less than current time");
            }
    ...
    public ApiCaller(String domainName, String  domainCode, ApiCtlPermission permission, Long createTime, Long expiredTime){
        super();
        Assert.notNull(domainCode);
        Assert.notNull(permission);
        if (createTime == null) {
            ...
            if (createTime < 0) {
                throw new IllegalArgumentException(expiredTime + " is not a valid create time, because it is a negative number");
            }
