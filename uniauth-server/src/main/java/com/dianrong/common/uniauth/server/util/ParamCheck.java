...
public class ParamCheck {
    ...
    public static void checkPageParams(Integer pageNumber, Integer pageSize, int count) {
        if (AppConstants.MIN_PAGE_NUMBER.equals(pageNumber) && AppConstants.MAX_PAGE_SIZE.equals(pageSize) && count > AppConstants.MAX_PAGE_SIZE) {
            throw new AppException(InfoName.BAD_REQUEST, UniBundle.getMsg("data.query.result.number.exceed", AppConstants.MAX_PAGE_SIZE, count));
        }
    }
}
