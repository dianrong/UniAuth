...
public class CasMultiTimeUseOrTimeoutExpirationPolicy extends AbstractCasExpirationPolicy {
     ...
     */
    public CasMultiTimeUseOrTimeoutExpirationPolicy(final int numberOfUses, final long timeToKillInMilliSeconds) {
     ...
     */
    public CasMultiTimeUseOrTimeoutExpirationPolicy(final int numberOfUses, final long timeToKill, final TimeUnit timeUnit) {
    ...
    private int getStUsedTimes() {
        int useTimes = NumberUtils.toInt(allZkNodeMap.get(AppConstants.ZK_NODE_NAME_ST_USED_TIMES), this.numberOfUses);
