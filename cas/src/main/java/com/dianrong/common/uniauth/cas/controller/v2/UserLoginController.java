...
public class UserLoginController {
  ...
  private static final Map<Class<? extends Exception>, CodeMessage> EXCEPTION_CODE_MAP = new HashMap<Class<? extends Exception>, CodeMessage>() {
...

    {
      ...
      put(UserPasswordNotMatchException.class,
          new CodeMessage(ResponseCode.USER_NAME_PSWD_NOT_MATCH,
              "api.v2.login.account.pssword.not.match"));
      put(MultiUsersFoundException.class,
          new CodeMessage(ResponseCode.MULTIPLE_USERS_FOUND, "api.v2.login.multiple.user.found"));
      ...
      put(FreshUserException.class,
          new CodeMessage(ResponseCode.ACCOUNT_NEED_INIT_PSWD, "api.v2.login.account.need.init"));
  ...
  public ApiResponse<String> login(
    ...
    UsernamePasswordCredential credential = new CasUsernamePasswordCredential(identity, password,
        null, tenancyCode);
    try {
      ...
      if (service == null) {
        ...
        String serviceTicket = loginSupport
            .loginAndQueryTicket(credential, false, request, response);
      ...
      return ApiResponse
          .failure(codeMessage.code, UniBundleUtil.getMsg(messageSource, codeMessage.messageCode));
    }
  }
