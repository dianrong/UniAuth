...
public class GenerateCustomLoginUrlAction {
  ...
  public final String generate(final RequestContext context) {
    ...
    String lt = (String) request.getSession().getAttribute(CasConstants.CAS_CUSTOM_LOGIN_LT_KEY);
    // 获取客户自定义页面url
    String baseUrl = (String) context.getFlowScope().get(CasConstants.CAS_CUSTOM_LOGIN_PAGE_KEY);
    // 以上的值 必须都存在
    if (baseUrl.contains("?")) {
      baseUrl += "&";
    } else {
    ...
    context.getFlowScope().put(CasConstants.CAS_CUSTOM_LOGIN_URL_KEY, baseUrl);
    return "generated";
  }
}
