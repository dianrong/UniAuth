...
public class CasLoginSupport {
     ...
     */
    public String grantServiceTicket(HttpServletRequest request, String ticketGrantingTicketId) throws TicketException, ValidateFailException {
     ...
     * @throws AuthenticationException 登陆失败了
     */
    public String loginAndQueryTicket(Credential credential, boolean warnCookie, HttpServletRequest request, HttpServletResponse response)
    ...
    public boolean isLoginRedirect(HttpServletRequest request) {
        boolean redirect = NEED_REDIRECT_VALUE.equalsIgnoreCase(request.getParameter(SERVICE_REDIRECT_PARAMETER));
        if (redirect) {
            Service service = queryService(request);
            if (service != null) {
                return true;
            }
            log.warn("request's parameter {} value is {}, but parameter service is null, please check request parameter", SERVICE_REDIRECT_PARAMETER, NEED_REDIRECT_VALUE);
        }
        return false;
    }
     ...
     */
    public boolean loginRedirect(HttpServletRequest request, HttpServletResponse response, String serviceTicket) throws IOException {
        ...
        if (!StringUtil.strIsNullOrEmpty(targetUrl)) {
            url.append("&").append(CasConstants.TARGET_URL_PARAMETER).append("=").append(targetUrl.trim());
        }
