        ...
        <form:select id="domain" tabindex="0" accesskey="${domainAccessKey}" path="domain">
          <c:if test="${not empty domains}">
            <c:forEach items="${domains}" var="domain">
              <form:option value="${domain.zkDomainUrlEncoded}"
                customLoginPage="${domain.isCustomizedLoginPage}" domainCode="${domain.code}">${domain.code}
              </form:option>
            </c:forEach>
          </c:if>
        </form:select>
          ...
          %>
            <section class="row">
              ...
              <div class="captcha-class">
                ...
                <a href="javascript:void(0);" tabindex="4" id="cas_login_captcha_change_a">
                  <spring:message code="screen.welcome.button.captcha.change"/>
                </a>
              </div>
            </section>
          <%
